...
export default function BookingDetail({ booking }: { booking: Booking }) {
  ...
  // <-- Minimal replacement for the "throw new Error" you had:
  function handleSelectSpot(spot: any): void {
    (async () => {
      try {
        // Update only spotId in the booking
        await updateBooking({
          id: booking.id,
          spotId: spot.id,
  ...
  return (
    <Card className="flex flex-col overflow-hidden justify-between h-screen">
      <CardContent className="p-0 text-sm h-[95%] relative">
        ...
        <Tabs className="mt-8 flex flex-col overflow-y-auto h-dvh" defaultValue="resume">
          <div className="mx-6">
                  ...
                  <dl className="grid gap-3">
                    {[
                      ...
                      return (
                        <div key={field} className="flex items-start justify-between">
                          <dt className="text-muted-foreground">{fieldName}</dt>
                          <dd>
                            {editingField === field ? (
                              <>
                              ...
                              <>
                                <div className="flex justify-between items-center relative gap-2">
                                  ...
                                  >
                                    {field === "status"
                                      ? statuses.find(
                                          (s) => s.value === getValues("status")
                                        )?.label
                                      : String(getValues(field as any))}
                                  </span>
                                  <span
                    ...
                    {getValues("customFields")?.map((field, index) => {
                      const customField = customFields.find(
                        (fd: { id: string }) => fd.id === field.customFieldId
                      );
                              ...
                              <>
                                <div className="flex justify-between items-center relative gap-2">
                                  <span
                                    ...
                                    onClick={() =>
                                      startEditing(`customFields.${index}.value`)
                                    }
                                  >
                                    {field.value ||
                                      customField?.placeholder ||
                ...
                <ul className="grid gap-3">
                  <form onSubmit={handleSubmit(onSubmit)}>
                    <dl>
                      <div className="flex items-start justify-between">
                        ...
                        <dd className="flex justify-between items-center gap-2">
                          {workspaceId && (
                            <SpotSelector
                              workspaceId={workspaceId}
                              defaultSpot={booking?.spot}
                  ...
                  <li className="flex items-center justify-between">
                    <span className="text-muted-foreground">
                      Total Extra Items x<span>1</span>
                    </span>
          ...
          <CarouselContent>
            ...
            <CarouselItem>
              <div className="text-xs text-muted-foreground">
                Created {moment(booking?.createdAt).format("DD MMMM YYYY hh:mm A")}
              </div>
            </CarouselItem>
            <CarouselItem>
              <div className="text-xs text-muted-foreground">
                Updated {moment(booking?.updatedAt).format("DD MMMM YYYY hh:mm A")}
              </div>
            </CarouselItem>
          </CarouselContent>
          <div>
