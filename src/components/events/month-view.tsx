...
import { format, addDays, startOfMonth, getDay, isSameDay, isSameMonth } from 'date-fns'
...
export default function MonthView({
    ...
    const daysToRender = Array.from({ length: 35 }, (_, i) => addDays(startDate, i - firstDayOfWeek));
          ...
          <div className="max-w-7xl w-full">
            ...
            <div className="grid grid-cols-7 gap-1">
              {daysToRender.map((date, index) => {
                ...
                const eventsForDate = events.filter((event) => isSameDay(new Date(event.startDate), date));
                ...
                return (
                  ...
                  >
                    <time dateTime={format(date, "yyyy-MM-dd")} className="text-xs font-semibold">
                      {format(date, "d")}
                    </time>
                    ...
                    <div className="flex flex-col justify-between h-full">
                            ...
                            <Tooltip key={event.id}>
                              <TooltipTrigger asChild>
                                <Link href={`/calendar/${event.id}`}>
                                  <div className="truncate bg-violet-500 text-white rounded px-1 py-0.5 mb-1 cursor-pointer"
                      ...
                      {remainingEventsCount > 0 && (
                        <div className="flex justify-center mt-1">
                          <button className="text-xs text-gray-400">+{remainingEventsCount} more</button>
                        </div>
                      )}
                    </div>
